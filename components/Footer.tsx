...
const Footer = ({ variant = 'dev' }: FooterProps) => {
  ...
  return (
    <footer className={`w-full py-12 border-t mt-20 transition-colors duration-500 ${
      isMusic ? 'border-purple-500/10 bg-white' : 'border-zinc-900/50 bg-black'
    }`}>
      <div className={`max-w-5xl mx-auto px-6 ${isMusic ? 'pl-20' : ''} flex flex-col items-center justify-center gap-4`}>
        ...
        <p className={`text-[9px] font-mono uppercase tracking-widest ${
          isMusic ? 'text-zinc-300' : 'text-zinc-700'
        }`}>
