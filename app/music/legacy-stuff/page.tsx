"use client";

import { useState } from "react";
import { MusicGenreLayout } from "@/components/MusicGenreLayout";
import { LegacyBackground } from "@/components/Backgrounds";
import { Track } from "@/components/CustomAudioPlayer";

const TRACKS: Track[] = [
  { id: "1", title: "Bollywood Strings", src: "/audio/legacy-stuff/track01.mp3", tag: "Orchestral" },
  { id: "2", title: "Akuma", src: "/audio/legacy-stuff/track02.mp3", tag: "Electronic" },
  { id: "3", title: "Determination", src: "/audio/legacy-stuff/track03.mp3", tag: "Dark" },
  { id: "4", title: "Enemy", src: "/audio/legacy-stuff/track04.mp3", tag: "Intense" },
];

export default function LegacyStuffPage() {
  const [activeTrack, setActiveTrack] = useState<string | null>(null);

  return (
    <MusicGenreLayout
      title="Legacy Stuff"
      description="Archived experiments and early explorations from my formative years of the sonic journey."
      tracks={TRACKS}
      theme="teal"
      themeColor="#0d9488"
      bgVariant="legacy"
      background={<LegacyBackground />}
      rootBgColor="bg-teal-50"
      entryColor="#0d9488"
      activeTrackId={activeTrack}
      setActiveTrack={setActiveTrack}
    >
      <div className="w-full flex justify-center mb-12">
        <div className="flex flex-wrap gap-6 justify-center">
          {TRACKS.map((track) => {
            const isActive = activeTrack === track.id;
            return (
              <button
                key={track.id}
                onClick={() => setActiveTrack(track.id)}
                className={`w-56 md:w-72 h-16 md:h-20 flex flex-col items-center justify-center rounded-2xl uppercase tracking-[0.25em] font-medium text-xs md:text-sm transition-all duration-300 border backdrop-blur-md ${isActive
                  ? 'bg-white text-teal-700 border-white shadow-[0_10px_40px_rgba(13,148,136,0.15)] scale-105 z-10'
                  : 'bg-white/10 text-teal-900/60 border-white/20 hover:bg-white/30 hover:text-teal-800 hover:shadow-lg hover:-translate-y-1'
                  }`}
              >
                <span className={isActive ? 'font-bold' : ''}>{track.title}</span>
                {track.tag && (
                  <span className={`text-[9px] mt-1 tracking-wider font-normal ${isActive ? 'text-teal-500/70' : 'text-teal-900/30'}`}>
                    {track.tag}
                  </span>
                )}
              </button>
            );
          })}
        </div>
      </div>
    </MusicGenreLayout>
  );
}