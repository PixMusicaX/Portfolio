"use client";

import { useState } from "react";
import { MusicGenreLayout } from "@/components/MusicGenreLayout";
import { OrchestralBackground } from "@/components/Backgrounds";
import { Track } from "@/components/CustomAudioPlayer";

const TRACKS: Track[] = [
  { id: "1", title: "Overture", src: "/audio/orchestral/track01.mp3", tag: "Fast" },
  { id: "2", title: "Castle", src: "/audio/orchestral/track02.mp3", tag: "Slow" },
  { id: "3", title: "Haunting", src: "/audio/orchestral/track03.mp3", tag: "Emotional" },
  { id: "4", title: "Finale", src: "/audio/orchestral/track04.mp3", tag: "Original" },
];

export default function OrchestralPage() {
  const [activeTrack, setActiveTrack] = useState<string | null>(null);

  return (
    <MusicGenreLayout
      title="Orchestral"
      description="Full symphonic arrangements with sweeping strings, bold brass, and symphonic pianos."
      tracks={TRACKS}
      theme="indigo"
      themeColor="#4f46e5"
      bgVariant="orchestral"
      background={<OrchestralBackground />}
      rootBgColor="bg-indigo-50"
      entryColor="#4f46e5"
      activeTrackId={activeTrack}
      setActiveTrack={setActiveTrack}
    >
      <div className="w-full flex justify-center mb-12">
        <div className="flex flex-wrap gap-6 justify-center">
          {TRACKS.map((track) => {
            const isActive = activeTrack === track.id;
            return (
              <button
                key={track.id}
                onClick={() => setActiveTrack(track.id)}
                className={`w-56 md:w-72 h-16 md:h-20 flex flex-col items-center justify-center rounded-2xl uppercase tracking-[0.25em] font-medium text-xs md:text-sm transition-all duration-300 border backdrop-blur-xl ${isActive
                  ? 'bg-white text-indigo-700 border-white shadow-[0_10px_40px_rgba(79,70,229,0.15)] scale-105 z-10'
                  : 'bg-white/20 text-indigo-900/60 border-white/20 hover:bg-white/30 hover:text-indigo-800 hover:shadow-lg hover:-translate-y-1'
                  }`}
              >
                <span className={isActive ? 'font-bold' : ''}>{track.title}</span>
                {track.tag && (
                  <span className={`text-[9px] mt-1 tracking-wider font-normal ${isActive ? 'text-indigo-500/70' : 'text-indigo-900/30'}`}>
                    {track.tag}
                  </span>
                )}
              </button>
            );
          })}
        </div>
      </div>
    </MusicGenreLayout>
  );
}