  ...
  return (
    <div className="min-h-screen bg-amber-50 text-amber-900 font-[family-name:var(--font-outfit)] relative overflow-hidden flex">
      ...
      <nav className="w-16 fixed left-0 top-0 bottom-0 flex flex-col justify-center items-center z-40 pointer-events-none">
        <div className="absolute top-[-10%] bottom-[-10%] left-[-100px] w-[160px] rounded-[50%] bg-white/40 border-r border-amber-500/10 shadow-[20px_0_50px_rgba(217,119,6,0.05)] backdrop-blur-sm z-[-1]" />

        <Link
          href="/music#genres"
          className="group flex flex-col items-center gap-6 text-[10px] sm:text-xs font-bold tracking-[0.3em] uppercase text-amber-400 hover:text-amber-600 hover:drop-shadow-[0_0_12px_rgba(217,119,6,0.4)] transition-all duration-300 pointer-events-auto"
        >
            ...
            {TRACKS.map((track) => {
              ...
              return (
                ...
                >
                  ...
                  {track.tag && <span className={`text-[10px] mt-1 tracking-wider font-normal ${isActive ? 'text-amber-500/70' : 'text-amber-900/30'}`}>{track.tag}</span>}
                </button>
              );
            })}
          </div>
      ...
      {/* Bottom Flame */}
      <GaseousDivider hoveredSide="left" variant="fusion" align="bottom" className={`transition-opacity duration-1000 ease-in-out ${isPlaying ? 'opacity-100' : 'opacity-0'}`} />
